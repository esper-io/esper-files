...
package io.esper.videos.fragment
...
import io.esper.videos.activity.ImageViewerActivity
import io.esper.videos.activity.VideoViewerActivity
import io.esper.videos.adapter.ItemAdapter
import io.esper.videos.adapter.ItemAdapter.ClickListener
import io.esper.videos.adapter.VideoURLAdapter
import io.esper.videos.async.LoadFileAsync
import io.esper.videos.callback.OnLoadDoneCallback
import io.esper.videos.constants.Constants.EsperScreenshotFolder
import io.esper.videos.constants.Constants.InternalCheckerString
import io.esper.videos.constants.Constants.InternalScreenshotFolderDCIM
import io.esper.videos.constants.Constants.InternalScreenshotFolderPictures
import io.esper.videos.constants.Constants.ListItemsFragmentTag
import io.esper.videos.constants.Constants.ORIGINAL_SCREENSHOT_STORAGE_PREF_KEY
import io.esper.videos.constants.Constants.ORIGINAL_SCREENSHOT_STORAGE_VALUE
...
import io.esper.videos.model.VideoURL
import io.esper.videos.util.FileUtils
import io.esper.videos.util.FileUtils.Decompress
    ...
    private fun openItem(selectedItem: Item) {
        ...
        if (selectedItem.isDirectory) {
            ...
            if (selectedItem.name!!.endsWith(".pdf", true)) {
                isPdf = true
                if (sharedPref!!.getBoolean(SHARED_MANAGED_CONFIG_USE_INBUILT_PDF, true)) {
                    try {
                        startActivity(
                        ...
                        Toast.makeText(context, "Sorry, Couldn't open up PDF!", Toast.LENGTH_SHORT)
                            .show()
                    }
                } else {
                    FileUtils.openFile(requireContext(), File(selectedItem.path))
